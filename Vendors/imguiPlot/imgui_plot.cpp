...
#include "imguiPlot/imgui_plot.hpp"
...
namespace ImGui {
// [0..1] -> [0..1]
static float rescale(float t, float min, float max,
                     PlotConfig::Scale::Type type) {
  switch (type) {
...
// [0..1] -> [0..1]
static float rescale_inv(float t, float min, float max,
                         PlotConfig::Scale::Type type) {
  switch (type) {
  case PlotConfig::Scale::Linear:
    return t;
...
static int cursor_to_idx(const ImVec2& pos, const ImRect& bb,
                         const PlotConfig& conf, float x_min, float x_max) {
  const float t =
      ImClamp((pos.x - bb.Min.x) / (bb.Max.x - bb.Min.x), 0.0f, 0.9999f);
  const int v_idx = (int)(rescale_inv(t, x_min, x_max, conf.scale.type) *
                          (conf.values.count - 1));
...
PlotStatus Plot(const char* label, const PlotConfig& conf) {
  ...
  if (conf.values.count > 0) {
    ...
    if (conf.tooltip.show && hovered && inner_bb.Contains(g.IO.MousePos)) {
      const int v_idx =
          cursor_to_idx(g.IO.MousePos, inner_bb, conf, x_min, x_max);
      ...
      const float y0 = ys_list[0][data_idx]; // TODO: tooltip is only shown for
                                             // the first y-value!
    ...
    if (conf.grid_x.show) {
      ...
      case PlotConfig::Scale::Linear: {
        float cnt =
            conf.values.count / (conf.grid_x.size / conf.grid_x.subticks);
        float inc = 1.f / cnt;
        for (int i = 0; i <= cnt; ++i) {
          int x0 = ImLerp(inner_bb.Min.x, inner_bb.Max.x, i * inc);
          window->DrawList->AddLine(
              ImVec2(x0, y0), ImVec2(x0, y1),
    ...
    if (conf.grid_y.show) {
      ...
      float cnt = (conf.scale.max - conf.scale.min) /
                  (conf.grid_y.size / conf.grid_y.subticks);
      float inc = 1.f / cnt;
      for (int i = 0; i <= cnt; ++i) {
    ...
    for (int i = 0; i < ys_count; ++i) {
      ...
      ImVec2 tp0 =
          ImVec2(t0, 1.0f - ImSaturate((v0 - conf.scale.min) * inv_scale));
      ...
      for (int n = 0; n < res_w; n++) {
        ...
        const float v1 =
            ys_list[i][conf.values.offset + (v1_idx + 1) % conf.values.count];
        const ImVec2 tp1 =
            ImVec2(rescale(t1, x_min, x_max, conf.scale.type),
        ...
        window->DrawList->AddLine(pos0, pos1, col_base, conf.line_thickness);
    ...
    if (conf.selection.show) {
      ...
      ImVec2 pos1 = ImLerp(inner_bb.Min, inner_bb.Max,
                           ImVec2(fSelectionStep * (*conf.selection.start +
                                                    *conf.selection.length),
                                  1.f));
  ...
  if (conf.overlay_text)
    RenderTextClipped(
        ...
        frame_bb.Max, conf.overlay_text, NULL, NULL, ImVec2(0.5f, 0.0f));
